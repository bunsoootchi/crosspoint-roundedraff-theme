...
void HomeActivity::loadRecentCovers(int coverHeight) {
  ...
  for (RecentBook& book : recentBooks) {
    if (!book.coverBmpPath.empty()) {
      if (isRoundedRaff) {
        // RoundedRaff prefers the higher-quality 2-bit cover.bmp (not 1-bit thumbs).
        // Only generate it once when missing; otherwise returning to Home should remain fast.
        if (StringUtils::checkFileExtension(book.path, ".epub")) {
          Epub epub(book.path, "/.crosspoint");
          const std::string coverBmpPath = epub.getCoverBmpPath(false);
          if (!Storage.exists(coverBmpPath.c_str())) {
            // generateCoverBmp() requires the metadata cache to be loaded (cover image href).
          ...
          if (!Storage.exists(coverBmpPath.c_str()) && xtc.load()) {
            if (!showingLoading) {
              showingLoading = true;
              popupRect = GUI.drawPopup(renderer, tr(STR_LOADING_POPUP));
            }
